...
- Docker Hub account (`your-dockerhub-user`)
- Minikube & kubectl
...
1. **Build & tag**
   ...
   docker build -t your-dockerhub-user/spfa:latest .
   ```
2. **Push to Docker Hub**
   ```bash
   docker push your-dockerhub-user/spfa:latest
   ```
...
4. **Or without Compose**
   ```bash
   docker run -p 5000:5000 your-dockerhub-user/spfa:latest
   ```
5. **Verify**  
...
> The app image is **docker.io/your-dockerhub-user/spfa:latest**.  
> The Helm chart is **oci://registry-1.docker.io/your-dockerhub-user/spfa** (version **0.1.1**).
...
### Windows PowerShell
```powershell
helm upgrade --install spfa oci://registry-1.docker.io/your-dockerhub-user/spfa `
  --version 0.1.1 `
  --set image.repository=your-dockerhub-user/spfa `
...
### macOS/Linux (bash)
```bash
helm upgrade --install spfa oci://registry-1.docker.io/your-dockerhub-user/spfa   --version 0.1.1   --set image.repository=your-dockerhub-user/spfa   --set image.tag=latest   --wait --atomic --timeout 180s
```
...
```bash
helm upgrade spfa oci://registry-1.docker.io/your-dockerhub-user/spfa   --version 0.1.1   --set image.repository=your-dockerhub-user/spfa   --set image.tag=<newTag>   --wait --atomic --timeout 180s
```
...
- Chart (OCI): `oci://registry-1.docker.io/your-dockerhub-user/spfa` (version `0.1.1`)
- App Image: `docker.io/your-dockerhub-user/spfa:latest`
